	...
	<head>
		...
		<script type="text/javascript">
			layui.config({
				...
				//业务类型加载
				ajaxTools.loadCodeDataKK($('#ue-ywlx'),{dmmc:'ywlx',bz:'1'},false,'','code.do?method=selectListCode','请选择业务类型');
				form.render('select');
				//关闭按钮
						...
						complate: function() { //完成的回调
							...
							$('#content').children('tr').each(function() {
								...
								$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
										openFlowEdit($(this).data('lsh'),$(this).data('ywlx'));
								});
							});	
				...
				function openFlowEdit(lsh,ywlx){
				var condition ='&cxdh=020106&lsh='+lsh+'&ymmc=start_edit&ywlx='+ywlx;
				layer.open({
					type: 2,
					id:'startEdit',
					content:['flow.do?method=forwardFlowEditPage'+condition],
					title: '车辆出场信息', /*<i class="fa fa-user" style="font-size:18px;color:#1AA094;"></i> */
					shade: 0.2,
					offset: ['30px', '15%'],
					area: ['1000px', '550px'],
					zIndex: 10000000,
	...
	<body>
		...
		<script type="text/html" id="tpl">
			...
			<tr data-lsh="{{ item.lsh }}" data-ywlx="{{ item.ywlx }}">
				...
				<td class="td9"><a href="javascript:;" data-lsh="{{ item.lsh }}" data-ywlx="{{ item.ywlx }}" data-opt="edit"    class="layui-btn layui-btn-mini"><i class="layui-icon">&#xe642;</i> 编辑</a></td>
			</tr>
			{{# }); }}
		</script>
	</body>
