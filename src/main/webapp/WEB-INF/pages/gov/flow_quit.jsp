...
<!-- 					<input type="text" id="flow-glbm" readonly placeholder="请选择申请部门" class="layui-input" style="width: 160px;"> -->
		...
		<script type="text/html" id="tpl">
			...
			<tr data-ywlx="{{ item.ywlx }}" data-lsh="{{ item.lsh }}">
				...
				<td class="td7">{{ formateYwgw(item.ywgw==null?'':item.ywgw) }}</td>
				<td class="td8">{{ formateShortDate(item.sqrq==null?'':item.sqrq) }}</td>
				
				<td class="td9">
					<a href="javascript:;" data-lsh="{{ item.lsh }}" data-jsryzjhm="{{item.jsryzjhm}}" data-hphm="{{item.hphm}}" data-jsry="{{item.jsry}}" data-opt="quit" class="layui-btn layui-btn-danger layui-btn-mini"><i class="layui-icon">&#xe640;</i> 退办</a>
				</td>
			</tr>
			{{# }); }}
		</script>
		...
		<script type="text/javascript" src="gov/js/flow_quit.js"></script>
