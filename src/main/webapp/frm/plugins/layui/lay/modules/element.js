...
layui.define('jquery', function(exports){
  ...
  Element.prototype.tabAdd = function(filter, options){
    ...
    titElem.append('<li lay-id="'+ (options.id||'') +'" data-cxid="'+(options.cxid||'')+'">'+ (options.title||'unnaming') +'</li>');
  ...
  ,call = {
    ...
    ,showChild: function(){
      ...
      if(parents.hasClass(NAV_TREE)){
        ...
        if($('#admin-side').hasClass('own-side-layout-extend')){
    ...
    //展开三级菜单
    ,showThirdChild: function(){
    	var othis = $(this);
      	var parent = othis.parent(), child = othis.next('dl.three-level-child');
        child.removeClass(NAV_ANIM);
        parent[child.css('display') === 'none' ? 'addClass': 'removeClass']('layui-nav-itemed-third');
    }
  ...
  Element.prototype.init = function(type){
    var that = this, items = {
      ...
      ,nav: function(){
        ...
        $(NAV_ELEM).not('.not-bottom-hover-bar').each(function(){
          var othis = $(this)
          ...
          itemElem.each(function(){
            ...
            child.children('dd').each(function(){
            	if($(this).find('dd')[0] && !$(this).find('.'+NAV_MORE)[0]){
            		$(this).children('a').append('<span class="'+ NAV_MORE +'"></span>');
            	}
            });
            ...
            var thirdA = child.children('dd').children('a');
            thirdA.off('click',call.showThirdChild).on('click',call.showThirdChild);
			      thirdA.next().children('dd').off('click', call.clickChild).on('click', call.clickChild);
          });
        });
      }
