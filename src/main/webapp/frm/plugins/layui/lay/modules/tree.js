...
layui.define('jquery', function(exports){
  ...
  Tree.prototype.tree = function(elem, children){
    ...
    var currSelectedNodeGlbm = options.currSelectedNodeGlbm
    	,currSelectedNodeSjbm = options.currSelectedNodeSjbm
    	,isCurrSelectedNodeGlbm = false
    	,isCurrSelectedNodeSjbm = false
    	,myclicked = "my-tree-node-clicked";
    
    layui.each(nodes, function(index, item){
      try {
		isCurrSelectedNodeGlbm = (item.tags.glbm == currSelectedNodeGlbm);
		//isCurrSelectedNodeSjbm = (item.tags.glbm == currSelectedNodeSjbm);
	} catch (e) {
		isCurrSelectedNodeGlbm = false;
		isCurrSelectedNodeSjbm = false;
		throw e;
	...
	var hasChild = item.children && item.children.length > 0;
      var ul = $('<ul class="'+ ((item.spread||isCurrSelectedNodeSjbm) ? "layui-show" : "") +'"></ul>');
      var li = $(['<li '+ ((item.spread||isCurrSelectedNodeSjbm) ? 'data-spread="'+ item.spread +'"' : '') +' '+(isCurrSelectedNodeGlbm?'class="'+myclicked+'"':'')+' >'
        //展开箭头
        ,function(){
          return hasChild ? '<i class="layui-icon layui-tree-spread">'+ (
        	(item.spread||isCurrSelectedNodeSjbm) ? icon.arrow[1] : icon.arrow[0]
          ) +'</i>' : '';
        }()
        ...
        ,function(){
          ...
          + ('<i class="layui-icon layui-tree-'+ (hasChild ? "branch" : "leaf") +'">'+ (
            hasChild ? (
              (item.spread||isCurrSelectedNodeSjbm) ? icon.branch[1] : icon.branch[0]
            ) : icon.leaf
          ) +'</i>') //节点图标
  ...
  Tree.prototype.click = function(elem, item){
    ...
    elem.children('a').on('click', function(e){
      ...
      var myclicked = "my-tree-node-clicked";
      var p = $(elem).parents('.layui-box.layui-tree')[0];
      $(p).find('li.my-tree-node-clicked').removeClass(myclicked);
      elem.addClass(myclicked);
