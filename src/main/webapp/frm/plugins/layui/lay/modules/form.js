...
layui.define('layer', function(exports){
  ...
  ,Form = function(){
    this.config = {
      verify: {
    	...
    	,money: [
           /(^[1-9]([0-9]{1,5})?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/
           ,'只能填写正确的格式,如:99.99或99'
        ]
    	,float8: [//可为空
             /(^[1-9]([0-9]{1,5})?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)|^$/
             ,'只能填写正确的格式,如:99.99或99'
	    ...
	    ,float10: [
	             /(^[1-9]([0-9]{1,7})?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/
	             ,'只能填写正确的格式,如:99.99或99'
         ]
	    ,float11: [//可为空
           /(^[1-9]([0-9]{1,7})?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)|^$/
        ...
        ,hphm1: [/^[A-Z_0-9]{5}$/
         ...
         ,money1: [/(^[1-9]([0-9]{1,10})?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/
                  ,'只能填写正确的格式,如:99.99或99'
               ]
      ...
      select: function(){
        ...
        ,events = function(reElem, disabled, isSearch){
          ...
          var showDown = function(){
            ...
            try {
            	var dlparents = dl.parents()
	            	,dloverflowpartent;
	            dlparents.each(function(){
	            	if($(this).css('overflow')!='visible'){
	            		dloverflowpartent = $(this);
	            		return false;
	            	}
	            });
	            
	            if(dloverflowpartent&&dloverflowpartent.length&&dloverflowpartent.length>0){
	            	var t = dl.css('top');
	            	var b = dl.css('bottom');
	            	...
	            	dl.css({'height':'auto','max-height':'300px'});
	
	            	tb = (tb=='auto'?'auto':parseInt(tb));
	            	
	            	if(dl.height()<(dloverflowpartent.height()-title.offset().top-title.height()-10)){
	            		dl.css({'top':tb,'bottom':'auto'});
	            	}else if(dl.height()<title.offset().top){
	    				dl.css({'bottom':tb,'top':'auto'});
	    			}else{
	    				if((title.offset().top-dloverflowpartent.offset().top-10+(parseInt(tb)/2))>(dloverflowpartent.height()/2)){
	    					dl.css({'bottom':tb,'top':'auto','max-height':((title.offset().top-dloverflowpartent.offset().top-10)+'px')});
	    				}else{
	    					dl.css({'top':tb,'bottom':'auto','max-height':((dloverflowpartent.height()-(title.offset().top-dloverflowpartent.offset().top+title.height()+10))+'px')});
	    				}
	    			}
	            }
			} catch (e) {
        ...
        selects.each(function(index, select){
          ...
          var value = select.value, selected = $(select.options[select.selectedIndex==-1?0:select.selectedIndex]); //获取当前选中项
  ...
  //单独的校验功能，从submit中提取出的
  Form.prototype.verifyForm = function(formElement){
	  var verify = form.config.verify, stop = null
	    ,DANGER = 'layui-form-danger', field = {} ,elem = $(formElement)
	    
	    ,verifyElem = elem.find('*[lay-verify]') // 获取需要校验的元素
	    ,formElem = formElement // 获取当前所在的form元素，如果存在的话
	    ...
	    layui.each(verifyElem, function(_, item){
	      ...
	      layui.each(ver, function(_, thisVer){
			...
			if(thisVer.indexOf('max')===0){
				var n = parseInt(thisVer.replace('max',''));
				tipMess = (le <= n ? '' : '值过长，最大长度'+n+'位');
			}else if(thisVer.indexOf('min')===0){
				var n = parseInt(thisVer.replace('min',''));
				tipMess = (le >= n ? '' : '值过短，最小长度'+n+'位');
			}else if(thisVer.indexOf('eq')===0){
				var n = parseInt(thisVer.replace('eq',''));
