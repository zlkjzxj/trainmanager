layui.define('layer' , function(exports){
  ...
  Upload.prototype.action = function(input, type){
    ...
    /**
     * 如果页面有一个‘(options.elem || '.layui-upload-file')+'-clone'’这个选择器的元素，
     * 则将有东西的file input移动到这个元素中，
     * 再将克隆的file input留在upload组件自己生成的form中
     */
    var elemClone = $((options.elem || '.layui-upload-file')+'-clone');
    //console.log(elemClone);
    if(elemClone&&elemClone.length&&elemClone.length>0){
    	var p = $(input).parent();
    	$(input).clone(true).appendTo(p);
    	elemClone.html('');
    	$(input).appendTo(elemClone);
    }else{
    	input.value = '';
    }
  };
