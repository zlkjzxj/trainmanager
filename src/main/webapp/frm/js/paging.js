...
layui.define(['layer', 'laypage', 'laytpl'], function(exports) {
	...
	Paging.prototype.init = function(options) {
		...
		if(_config.type.toUpperCase() !== 'GET' && _config.type.toUpperCase() !== 'POST') {
			throwError('Paging Error:type参数配置出错，只支持GET或POST');
		}
		that.get({
			pageIndex: 1,
	...
	Paging.prototype.reload = function(options){
		this.get({
			pageIndex: this.config.params.pageIndex,
	...
	Paging.prototype.get = function(options) {
		...
		$.ajax({
			...
			success: function(result, status, xhr) {
				if(_config.beforeRender) {
					if(!_config.beforeRender(result)){ //渲染之前 返回false，不再继续
						return;
					}
				}
